    ...
    if (empty($username) || empty($password)) {
        ...
        if (mysqli_stmt_fetch($stmt)) {
            ...
            if ($locked_until_dt && $locked_until_dt > $now) {
                ...
                $stmt2 = mysqli_prepare($dbconn, "UPDATE manager_creds SET failed_attempts=0, locked_until=NULL WHERE id=?");
                ...
                if ($failed_attempts >= 3 && $minutes_since_last_fail !== null && $minutes_since_last_fail <= 15) {
                    // Lock for 10 minutes
                    $lock_expires = $now->add(new DateInterval('PT10M'))->format('Y-m-d H:i:s');
                ...
                $stmt3 = mysqli_prepare($dbconn, "UPDATE manager_creds SET failed_attempts=?, last_failed_login=NOW(), locked_until=? WHERE username=?");
                mysqli_stmt_bind_param($stmt3, "iss", $failed_attempts, $locked_until_update, $username);
